...
ContentType._VALUES_TO_NAMES = (content) => {
  for (let obj in ContentType) {
    if (content === obj.typeNumber) {
      return obj.name;
    }
  }
}
...
class LineBase {
  ...
  sendImage(filepath, filename = 'Line Image') {
    ...
    return readFile(filepath).then((bufs) => {
      return this._client.sendMessage(message).then((responseMessage) => {
        let data = {
   ...
   */
  sendImageWithURL(url, filename = 'Line Image') {
    ...
    return _client.sendMessage(message).then((responseMessage) => {
      let unireq = unirest.get(url);
      unireq.options['encoding'] = 'binary';
      return unireq.end((resp) => {
        return tmpFilePromise({ postfix: '.jpg' }).then((path) => {
          return appendFile(path, resp.body, 'binary')
...
class LineRoom extends LineBase {
  ...
  invite(contact) {
    if (!(contact instanceof LineContact)) {
      return Promise.reject(new Error('You should pass a LineContact as parameter'));
    }
