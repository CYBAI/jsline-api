  ...
  login() {
    ...
    return loginPromise.then((result) => {
      ...
      return Promise.join(
        ...
        (lastOp, profile, contacts, groups, rooms) => { // eslint-disable-line no-unused-vars
        ...
        for (let i = 0, opLen = operations.length; i < opLen; i++) {
          ...
          switch (operation.type) {
            ...
            case OpType.RECEIVE_MESSAGE: {
              ...
              const rawSender = operation.message.from;
            ...
            default:
              console.log(`[*] ${OpType._VALUES_TO_NAMES[operation.type]}`);
              console.dir(operation);
              break;
          }
