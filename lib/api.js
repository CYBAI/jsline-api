...
OpType._VALUES_TO_NAMES = (op) => {
  for (let obj in OpType) {
    if (op === obj.typeNumber) {
      return obj.name;
    }
  }
}
...
export class LineAPI {
  ...
  _login(id, password) {
    ...
    .then ((rsaCrypto) => {
      ...
      this._client.loginWithIdentityCredentialForCertificate(
        this.provider, rsaCrypto.keyname, rsaCrypto.credentials, true, this.config.ip, this.config.hostname, rsaCrypto.message,
        (err, result) => {
          if (err) { console.log('LoginFailed'); console.error(err); throw (err); }
          console.log(result);
          this._client.pinCode = result.pinCode;
          this.alertOrConsoleLog(`Enter Pincode ${result.pinCode} to your mobile phone in 2 minutes`);
  ...
  _setProvider(id) {
    this.provider = this.config.EMAIL_REGEX.test(id) ? IdentityProvider.LINE : IdentityProvider.NAVER_KR;
    if (this.provider === IdentityProvider.LINE) {
